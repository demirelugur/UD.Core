        ...
        private ValidationResult tovalidationresult(ValidationContext validationContext)
        {
            if (this.ErrorMessage.IsNullOrEmpty())
            {
                if (this.isequal)
                {
                    ...
                    if (ValidationChecks.IsEnglishCurrentUICulture) { this.ErrorMessage = $"{validationContext.DisplayName} must be one of the values [{String.Join(", ", this.values)}]!"; }
                }
                else
                {
                    this.ErrorMessage = $"{validationContext.DisplayName}, [{String.Join(", ", this.values)}] değerleri dışında farklı bir değer olmalıdır!";
                    if (ValidationChecks.IsEnglishCurrentUICulture) { this.ErrorMessage = $"{validationContext.DisplayName} must be a different value than [{String.Join(", ", this.values)}]!"; }
                }
            }
