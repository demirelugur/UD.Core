namespace UD.Core.Helper.Validation
{
    ...
    public sealed class StrongPasswordValid
    {
        ...
        private bool checkNameSurname(string valueSeo, string nameOrSurname)
        {
            var values = nameOrSurname.ToStringOrEmpty().ToEnumerable().Select(x => (x == "" ? [] : x.Split(' ').Select(y => y.ToSeoFriendly()).Where(y => y != "").ToArray())).FirstOrDefault();
