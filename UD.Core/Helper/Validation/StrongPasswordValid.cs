namespace UD.Core.Helper.Validation
{
    ...
    public sealed class StrongPasswordValid
    {
        ...
        public bool TryIsWarning(string value, string name, string surname, out string[] errors)
        {
            ...
            {
                if (Guards.IsUICultureEnglish) { r.Add($"The password must have a minimum of {this.minimumLength} characters and contain at least 1 Uppercase Letter, 1 Lowercase Letter, 1 Number and 1 Punctuation mark!"); }
            ...
            {
                if (Guards.IsUICultureEnglish) { r.Add($"Password can be maximum {this.maximumLength.Value} characters!"); }
                else { r.Add($"Şifre maksimum {this.maximumLength.Value} karakter olabilir!"); }
            }
            if (this.isConsecutive && this.checkConsecutive(value))
            {
                if (Guards.IsUICultureEnglish) { r.Add("The password must not contain 3 consecutive numbers! (123, 987 etc...)"); }
                else { r.Add("Şifre içerisinde 3 ardışık sayı (123, 987 vb...) bulunmamalıdır!"); }
            }
            if (this.isEmpty && value.Contains(' '))
            {
                if (Guards.IsUICultureEnglish) { r.Add("There should be no empty characters in the password!"); }
            ...
            if (this.isTurkishSpecialCharacter && value.Any(ArrayConstants.TurkishSpecialCharacters.Contains))
            {
                ...
                if (Guards.IsUICultureEnglish) { r.Add($"The password must not contain any letters specific to the Turkish language! ({t})"); }
            ...
            {
                if (Guards.IsUICultureEnglish) { r.Add("Your name(s) must not appear in the password!"); }
                else { r.Add("Şifre içerisinde adınız/adlarınız geçmemelidir!"); }
            }
            if (this.checkNameSurname(passwordSeo, surname))
            {
                if (Guards.IsUICultureEnglish) { r.Add("Your surname(s) must not appear in the password!"); }
