    ...
    public sealed class TokenBlacklistMiddleware
    {
        ...
        public async Task Invoke(HttpContext httpContext)
        {
            ...
            if (!token.IsNullOrEmpty() && await tokenBlacklistService.Any(token))
            {
                ...
                await httpContext.Response.WriteAsJsonAsync(ApiResult.setFailed(ValidationChecks.IsEnglishCurrentUICulture ? "Token invalid!" : "Token geçersiz!"), httpContext.RequestAborted);
                return;
            }
